...
const Card = ({ data, key, id }) => {
   const [imageurl, setImageUrl] = useState([]);
   const bgColors = ["#EE8434", "#C95D63", "#AE8799", "#717EC3", "#496DDB"];
   ...
   return (
      <div
         ...
         style={{ backgroundColor: bgColors[id % bgColors.length] }}
         onClick={() => {
...
const BuyParking = () => {
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            ...
            notification.error({ message: "Error in fetching  parking" });
            console.error(err);
         }
      };
      if (Walletaddress !== "") {
   ...
   useEffect(() => {
      const filteredPlaces = placelist.filter(
         (place) =>
            place.name.toLowerCase().includes(places.toLowerCase()) ||
            place.place.toLowerCase().includes(places.toLowerCase())
      );
      setoriginalplace(filteredPlaces);
   }, [places]);
   return (
      <div>
         ...
         <div className="grid lg:grid-cols-3 md:grid-cols-2 grid-cols-1 justify-items-center w-5/6 mx-auto my-0 !gap-5 p-4 flex-wrap">
            {originalplace.length !== 0 &&
               originalplace?.map((parking, index) => (
                  <Card
                     data={parking}
                     key={index}
                     id={index}
